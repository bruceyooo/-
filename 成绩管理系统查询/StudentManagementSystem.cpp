...
void input()         //2.学生信息录入
{
   ...
   int a;
   cout << "-----录入学生信息-----" << endl;
   cout << "请输入信息，输入0 结束录入" << endl;
   cout << "依次输入:学号 姓名 班级 语文 英语 数学 C++" << endl;
   for (a = e; a < n; a++)
   {
      cin >> m [a].num;
      if (m [a].num == 0)
...
void sort4()   //7.4.按C++成绩排序
{
   ...
   for (i = 0; i < e - 1; i++)
   {
      for (j = 0; j < e - i - 1; j++)
      {
         if (m [j].less.cgrade < m [j + 1].less.cgrade)                      //冒泡排序
         {
            strcpy(t1, m [j].name);
            strcpy(t2, m [j].clas);
            t4 = m [j].less.chinesegrade;
...
void sort6()   //7.6.按总成绩由小到大排序 （冒泡排序）
{
   int   j, i;
   char  t1 [20], t2 [20];
   int   t4, t5, t6, t7;
...
void sort()   //7.学生成绩排序工具
{
   ...
   cout << "                          * 4. 按C++成绩排序       *" << endl;
   cout << "                          * 5. 按学号排序          *" << endl;
   cout << "                          * 6. 按总成绩由小到大排序*" << endl;
   cout << "                          * 7. 退出程序            *" << endl;
   cout << "                          ##########################" << endl;
   cout << "                          * 输入「1-7」进入对应功能*" << endl;
   int r;
   cin >> r;
   switch (r)
   {
...
void statistics()    //8.学生信息统计工具，计算最高分、最低分、平均分
{
   system("cls");    //系统清屏
   ...
   float maxc = m [0].less.chinesegrade,                        //语文最大值
         ...
         pm,
		 maxl = m [0].less.cgrade,								//C++最大值
         minl = m [0].less.cgrade,								//C++最小值
         pl,
         max_score        = maxc + maxe + maxm +maxl,                 //总分最大值
         min_score        = minc + mine + minm +minl,                 //总分最小值
         max_chinesegrade = 0,
         max_englishgrade = 0,
         max_mathgrade    = 0,
		 max_cgrade		  = 0,
   ...
   pl = count4;			//C++成绩赋值给  pl

   /*---显示提示---*/

   cout << "1. 计算某学生的总分、平均分" << endl;
   ...
   else         //每个学科的最低最高平均及格率
   {
	  ...
	  case 4:
         cout << "C++成绩最高分:  " << maxl << endl;
         cout << "C++成绩最低分:  " << minl << endl;
         cout << "C++全班平均分:  " << suml / e << endl;
         cout << "C++及格率:  " << pl / e << endl;
         break;
